...
developer gets the same `/wrap-up`, `/checkpoint`, `/init-project`, `/resume`,
`/next-step`, `/bug`, `/file-bug` and `/build-timer-note` behaviour — including the doc
timestamp standard (`YYYY-MM-DD HH:MM TZ`), the shared doc-style / bug-report guides, and
...
Managed items: skills = wrap-up checkpoint next-step; commands = checkpoint init-project resume ship bug file-bug build-timer-note; guides = doc-style-reference.md bug-report-style.md; scripts = link-doc-refs.py reflow-md.py; github = bug_report.yml config.yml.
Last synced: 2026-07-21 02:06 MST
