...
_Last updated 2026-08-20 12:12 MST by an AI session · transcript: `f0912a53-461b-4861-97e4-931cb2f83ba0` — the corpus guard now computes its own exceptions instead of skipping words_
...
- **Test suite: `tests/test_terms.py --corpus` 89 assertions, ⚠️ 1 failing** — `Ninthroot forces only variants the classifier refuses` (`9th route` / `9th grid` are redundant `force:` entries; the classifier already applies them). ⚠️ **From term data added by `/add-term`, not from code** — verified clean at HEAD by `git status`. Cosmetic. `tests/test_notion_corrections.py` — 16 assertions, 0 failures. The count of `force:` entries is no longer pinned — see [`LESSONS_LEARNED.md`](LESSONS_LEARNED.md).
